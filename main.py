...
def get_difficult():
    ...
    difficult_map = [
        ("新手",  "5~12", "相当", "较高", "0.3~0.6", "0.3~0.6", "简易", "少量低伤害、易察觉", "请提醒用户进行属性鉴定"),
        ("新手",  "6~13", "相当", "较高", "0.3~0.6", "0.4~0.8", "简易", "少量低伤害、易察觉", "请提醒用户进行属性鉴定"),
        ("简单",  "7~14", "相当", "较高", "0.5~0.75", "0.5~0.9", "简易", "一些低伤害", "请提醒用户进行属性鉴定"),
        ("简单",  "8~15", "相当", "较高", "0.6~0.75", "0.6~1.2", "中等", "一些低/中伤害", "请提醒用户进行属性鉴定"),
        ("普通",  "9~16", "相当", "较高", "0.7~1.0", "0.8~1.4", "中等", "一些低/中伤害", "90%概率提醒用户进行属性鉴定；10%概率如果用户没有主动鉴定，则直接触发机关并进行豁免检定"),
        ("普通",  "9~17", "较高", "极高", "0.8~1.0", "1.0~1.7", "较强", "大量低/中伤害", "80%概率提醒用户进行属性鉴定；20%概率如果用户没有主动鉴定，则直接触发机关并进行豁免检定"),
        ("困难", "10~17", "较高", "极高", "0.9~1.15", "1.3~2.0", "较强", "大量中/高伤害", "75%概率提醒用户进行属性鉴定；25%概率如果用户没有主动鉴定，则直接触发机关并进行豁免检定"),
        ("困难", "10~18", "较高", "极高", "1.0~1.25", "1.5~2.5", "较强", "大量中/高伤害", "70%概率提醒用户进行属性鉴定；30%概率如果用户没有主动鉴定，则直接触发机关并进行豁免检定"),
        ("极难", "11~18", "较高", "极高", "1.0~1.5", "2.0~3.0", "很强", "大量高伤害", "65%概率提醒用户进行属性鉴定；35%概率如果用户没有主动鉴定，则直接触发机关并进行豁免检定"),
        ("极难", "12~19", "较高", "极高", "1.0~1.7", "2.5~4.0", "很强", "大量高伤害", "60%概率提醒用户进行属性鉴定；40%概率如果用户没有主动鉴定，则直接触发机关并进行豁免检定"),
    ]
...
def DiceTool(count: int = 24) -> str:
    ...
    ret = f"""
System: 所有鉴定和攻击请依次使用下面骰子的随机结果（无须全部用完）：
d20: {" ".join(d20_list)}
d12: {" ".join(d12_list)}
d10: {" ".join(d10_list)}
d8: {" ".join(d8_list)}
...
async def get_model(
    ...
    group_instuction = "当前是单人玩家模式，由我操作玩家角色，由你托管团队其他成员。如果不在战斗中，由我进行团队决策，你可以提出选项；当在战斗的时候，依据先攻顺序建议玩家的行动次序并在玩家的回合让我行动。"
    if is_group:
